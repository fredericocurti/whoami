...
const renderHighlights = async () => {
  ...
  },
  {
    ...
    link: `https://github.com/fredericocurti/night-shift-slider`,
  ...
  cont.innerHTML = data.map(d => `
    <div class="highlight-item" href="${d.link}">
  ...
  document.querySelectorAll('.highlight-item').forEach(e => e.addEventListener('click', onClickHighlight))
}

const onClickHighlight = (ev) => {
  window.open(ev.target.parentNode.parentNode.attributes.href.value, '_blank').focus();
}
